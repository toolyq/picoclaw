...
# Staging workspace for embedding
Write-Host "[INFO] Staging workspace for embedding..."
$onboardWorkspace = "cmd/picoclaw/internal/onboard/workspace"
if (Test-Path -Path $onboardWorkspace) {
    Remove-Item -Path $onboardWorkspace -Recurse -Force
}
Copy-Item -Path "workspace" -Destination $onboardWorkspace -Recurse
