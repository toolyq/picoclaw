...
instructions: |
  ...
  ```powershell
  python "d:\git\picoclaw\workspace\skills\fetch-x-twitter\scripts\fetch-x-twitter.py" [n] --channel "<channel>" --chat_id "<chat_id>" [--reply_to_message_id "<message_id>"]
  ```
---
...
1. Determine the current `channel`, `chat_id`, and `reply_to_message_id` (if available) from your system context block (look for "Current Session").
