...
async def fetch_x_tweets(n=3, channel_id="", chat_id="", reply_to_message_id=""):
    """
...
if __name__ == "__main__":
    ...
    while i < len(args):
        ...
        if arg == "--json":
            ...
            chat_id = args[i+1]
            i += 1
        elif arg == "--reply_to_message_id" and i+1 < len(args):
            reply_to_message_id = args[i+1]
            i += 1
        elif arg.isdigit():
            count = int(arg)
        i += 1
    ...
    try:
        results = asyncio.run(fetch_x_tweets(count, channel_id, chat_id, reply_to_message_id))
        if not results:
